    ...
    for idx_set = 1:num_set
        ...
        for scale = scale_all
            ...
            for idx_ext = 1:length(ext)
                filepaths_HR = cat(1, filepaths_HR, dir(fullfile('./HR', dataset{idx_set}, ['x', num2str(scale)], ext{idx_ext})));
                filepaths_SR = cat(1, filepaths_SR, dir(fullfile('./SR', degradation, methods{idx_method}, dataset{idx_set}, ['x', num2str(scale)], ext{idx_ext})));
            end
            PSNR_all = zeros(1, length(filepaths_HR));
            SSIM_all = zeros(1, length(filepaths_HR));
            IFC_all = zeros(1, length(filepaths_HR));
            if length(filepaths_HR) ~= length(filepaths_SR)
                error('the number of images in .HR and ./SR is different.');
            end
            for idx_im = 1:length(filepaths_HR)
                ...
                if 3 == size(im_HR, 3) && 3 == size(im_SR, 3)
...
%                 im_SR_Y = single(im2double(im_SR));
                ...
                [PSNR_all(idx_im), SSIM_all(idx_im), IFC_all(idx_im)] = Cal_Y_Matrix(im_HR_Y*255, im_SR_Y*255, scale, scale, use_ifc);
                ...
                fprintf('x%d %d %s: PSNR= %f SSIM= %f IFC= %f\n', scale, idx_im, name_SR, roundn(PSNR_all(idx_im),-2), roundn(SSIM_all(idx_im),-4), IFC_all(idx_im));
            end
            fprintf(results, '--------Mean--------\n');
            fprintf('--------Mean--------\n');
            fprintf(results, 'x%d: PSNR= %f SSIM= %f IFC= %f\n', scale, mean(PSNR_all), mean(SSIM_all), mean(IFC_all));
            fprintf('x%d: PSNR= %f SSIM= %f IFC= %f\n', scale, roundn(mean(PSNR_all),-2), roundn(mean(SSIM_all),-4), mean(IFC_all));
        end
    end
end
fclose(results);
